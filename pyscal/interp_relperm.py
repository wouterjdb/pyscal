...
base:  # One unified file with SWOF and SGOF or two separate files. Both SWOF and SGOF are required for base
  - swof_base.inc
  - sgof_base.inc

high:  # Nothing required; can be omitted if only to interpolate between base and low
       # Can drop SGOF if interolating SWOF only and vice verca
  - swof_opt.inc
  - sgof_opt.inc

low:   # Nothing required; can be omitted if only to interpolate between base and high
       # Can drop SGOF if interolating SWOF only and vice verca
  - swof_pes.inc
  - sgof_pes.inc
...
interpolations:
  ...
  - tables : [2,4,5,75] # applies to satnum 2, 4, 5, and 75, and only to SWOF (ie not SGOF)
    param_w  :  0.5
...
def make_interpolant(
    ...
    Parameters:
        ...
        interp_param (dictionary('param_w', 'param_g')): containing the interpolation parameter values
        satnum (int) : the satuation number index
        has_high_SWOF : (bool) indicating if user has provided a high table for SWOF
        has_low_SWOF  : (bool) indicating if user has provided a low  table for SWOF
        has_high_SGOF : (bool) indicating if user has provided a high table for SGOF
        has_low_SWOF  : (bool) indicating if user has provided a low  table for SGOF
